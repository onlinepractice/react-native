...
// Runs ADB reverse tcp:${process.env.REACT_PACKAGER_PORT} tcp:${process.env.REACT_PACKAGER_PORT} to allow loading the jsbundle from the packager
function tryRunAdbReverse(device) {
  try {
    const adbPath = getAdbPath();
    const adbArgs = ['reverse', `tcp:${process.env.REACT_PACKAGER_PORT}`, `tcp:${process.env.REACT_PACKAGER_PORT}`];
